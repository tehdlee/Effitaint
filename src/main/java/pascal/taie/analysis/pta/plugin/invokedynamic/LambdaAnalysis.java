...
public class LambdaAnalysis implements Plugin {
    ...
    public void onNewCSMethod(CSMethod csMethod) {
        ...
        lambdaObjs.get(method).forEach(lambdaObj -> {
            ...
            solver.propagateNew(csManager.getCSVar(context, ret), context, lambdaObj);
        });
    }
...
//        solver.addCallEdge(callEdge);
    ...
    public void onNewCallEdge(Edge<CSCallSite, CSMethod> edge) {
        if (edge instanceof LambdaCallEdge lambdaCallEdge) {
            ...
            int j = 0;
            for (int i = shiftC; i < capturedArgs.size() && targetParams.size() > j; ++i, ++j) {
//                solver.addPFGEdge(new PointerFlowEdge(
            ...
            for (int i = shiftA; i < actualArgs.size() && targetParams.size() > j; ++i, ++j) {
                solver.addPFGEdgeAndPropagate(new PointerFlowEdge(
                        FlowKind.PARAMETER_PASSING,
