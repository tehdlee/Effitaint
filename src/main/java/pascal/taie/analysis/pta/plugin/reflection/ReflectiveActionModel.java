        ...
        fldObjs.forEach(fldObj -> {
            ...
            if (field != null) {
                if (field.isStatic()) {
                    ...
                    solver.addPFGEdgeAndPropagate(new PointerFlowEdge(FlowKind.STATIC_LOAD,sField,to),Identity.get());
                } else {
                    ...
                    baseObjs.forEach(baseObj -> {
                        ...
                        if (typeSystem.isSubtype(declType, objType)) {
                            ...
                            solver.addPFGEdgeAndPropagate(new PointerFlowEdge(FlowKind.INSTANCE_LOAD,ifield,to), Identity.get() );
                            allTargets.put(invoke, field);
                        }
                    });
                }
            }
    ...
    public void arrayNewInstance(Context context, Invoke invoke, PointsToSet pts) {
        ...
        pts.forEach(obj -> {
            ...
            if (baseType != null && !(baseType instanceof VoidType)) {
                ...
                solver.propagateNew(context, result, csNewArray);
        ...
        if (edge instanceof ReflectiveCallEdge refEdge) {
            ...
            if (result != null && isConcerned(callee.getReturnType())) {
                ...
                callee.getIR().getReturnVars().forEach(ret -> {
                    ...
                    solver.addPFGEdgeAndPropagate(new PointerFlowEdge(RETURN,csRet, csResult), Identity.get());
