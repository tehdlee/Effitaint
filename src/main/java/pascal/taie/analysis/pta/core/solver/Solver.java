...
public interface Solver {
    ...
    default void propagateNew(Pointer pointer, CSObj csObj){
        PointsToSet pts = makePointsToSet();
        pts.addObject(csObj);
        propagateNew(pointer,pts);
    }

    default void propagateNew(Pointer pointer, Context heapContext, Obj obj){
        PointsToSet pts = makePointsToSet();
        pts.addObject(getCSManager().getCSObj(heapContext,obj));
        propagateNew(pointer,pts);
    }
    ...
    PointsToSet getPtsArrayElems(Pointer pointer);

    boolean isContainedArray(Pointer pointer);
