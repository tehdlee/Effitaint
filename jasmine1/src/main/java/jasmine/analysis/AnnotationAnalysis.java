...
public class AnnotationAnalysis {
    ...
    public Integer getAllComponents(SootClass sootClass) {
        // 需要过滤掉的注解类名列表
        Set<String> annotationClassesToFilter = new HashSet<>(Arrays.asList(
        ...
        // 如果 sootClass 本身是需要过滤的注解类，直接返回，不做处理
        if (annotationClassesToFilter.contains(sootClass.getName())) {
            return 0;
        }
