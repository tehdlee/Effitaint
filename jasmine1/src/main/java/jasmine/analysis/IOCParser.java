...
//        尝试获取类的构造方法 <init>，如果有多个构造方法（导致歧义），则设置 ambiguous 标志。
        try {
            initMethod = sootClass.getMethodByName("<init>");
        } catch (AmbiguousMethodException amx) {
            ambiguous = true;
        }
//        遍历 sootClass 的所有方法，检查是否存在自动注入的方法参数。如果存在，记录这些参数。
...
//        遍历 sootClass 的所有字段，查找带有特定注解的字段。
//        对每个带注解的字段，尝试解析依赖并进行依赖注入建模。
...
//                aClass 的作用是存储和处理字段类型的具体实现类。
//                具体来说，aClass 被用来检查字段的类型是否是接口，并找到对应的实现类，以便进行依赖注入建模。
                // 根据字段（变量）的声明类型，和已经存在的bean列表，获取
                SootClass aClass = CreateEdge.interfaceToBeans.getOrDefault(((RefType) field.getType()).getSootClass().getName(), null);
                if (aClass != null && !aClass.isInterface()) {
//                    aClass 不是接口类型，处理为具体类
                    // Generate dynamic proxy
//                    使用 AOPParser.proxyMap 检查是否有代理类，若有则替换为代理类。
...
//                    如果 aClass 不在 CreateEdge.prototypeComponents 中，表示它是单例组件，
//                    调用 this.initIOCObjectBySingleton 方法进行单例注入建模。
